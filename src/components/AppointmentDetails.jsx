...
export default function AppointmentDetails() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Pre-select tests that were already saved
        setSelectedTests(appData.requiredTests?.map(t => t.testId) || []);
  ...
  const handleSaveDiagnosis = async () => {
    setSubmitting(true);
    try {
      await axios.post(`${API_BASE}/doctor-appointments/${id}/diagnoses`, {
        diagnosis,
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] shadow-xl p-8">
            <div className="flex justify-between items-center mb-8">
              ...
              <span className={`px-4 py-1.5 rounded-full text-[9px] font-black ${
                ...
                appointment?.status === 'Completed' || appointment?.status === 'Complete' ? 'bg-emerald-100 text-emerald-600' : 
            ...
            {/* Video Meeting Button — Online only, grayed out if not active time */}
            {appointment?.appointmentType === "Online" && appointment?.status !== "NoShow" && (
              <button 
                onClick={isMeetingActive() ? handleJoinMeeting : undefined}
                disabled={!isMeetingActive()}
                className={`w-full py-5 rounded-[1.5rem] flex items-center justify-center gap-3 font-black text-[11px] uppercase tracking-widest mb-4 transition-all ${
                  isMeetingActive()
                    ? "bg-blue-600 text-white shadow-lg hover:bg-blue-700 active:scale-95 cursor-pointer"
                    : "bg-slate-100 text-slate-400 cursor-not-allowed"
                }`}
              >
                <Video size={20} />
                {isMeetingActive() ? "Join Video Meeting" : "Cant Join Meeting Now"}
              </button>
            )}

            {/* No-Show button — hidden for completed appointments */}
            {appointment?.status === "Completed" && (
              <button 
                onClick={handleNoShow}
                className="w-full border-2 border-red-50 text-red-500 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase hover:bg-red-100 hover:text-red-600 active:scale-95 transition-all"
              >
